...
本文件說明 TopChurchPlus 如何使用本機 Ollama 做任務前置分析，目標是降低 Codex Token 消耗、加速定位相關檔案，並把簡單的「文件摘要、關鍵字搜尋、風險提示」交給 Local AI 先處理。
...
不建議。Local AI 適合做低風險、低權限、可重做的前置工作。實際修改、部署、資料庫變更、權限與資安判斷仍由 Codex 按專案流程處理。
